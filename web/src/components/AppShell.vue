...
import { Container, ExternalLink, Menu, Search, X, Zap } from 'lucide-vue-next'
...
const links = [
  { to: '/', label: 'Docker 镜像加速', icon: Container },
...
<template>
  <div class="shell-atmosphere flex min-h-screen flex-col text-foreground">
      ...
      <div class="mx-auto flex h-[4.25rem] max-w-6xl items-center justify-between gap-3 px-5 sm:px-8">
        ...
        <nav class="hidden items-center gap-1.5 md:flex">
          <RouterLink
            ...
            :key="link.to"
            :to="link.to"
            class="inline-flex items-center gap-1.5 rounded px-4 py-2 text-[15px] transition-colors duration-150"
          ...
          <a
            ...
            class="inline-flex items-center gap-1.5 rounded px-4 py-2 text-[15px] text-muted-foreground transition-colors duration-150 hover:bg-accent hover:text-foreground"
          >
        ...
        <div v-if="menuOpen" class="border-t border-border px-5 py-2 md:hidden">
          <div class="flex flex-col gap-1">
            <RouterLink
              ...
              :key="link.to"
              :to="link.to"
              class="inline-flex items-center gap-2 rounded px-3.5 py-2 text-[15px] transition-colors"
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              class="inline-flex items-center gap-2 rounded px-3.5 py-2 text-[15px] text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
              @click="closeMenu"
            >
              <ExternalLink class="size-4" />
              Atoman 主站
    ...
    <footer class="flex justify-center pb-10 pt-2 text-sm text-muted-foreground">
      Atoman Proxy
    </footer>
  </div>
</template>
